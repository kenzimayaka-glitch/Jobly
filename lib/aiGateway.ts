...
const OPS = new Set<AiOperation>(["INTERVIEW","LEARNING","APPLICATION_COPILOT","CAREER_COMPANION"]);
...
const sanitizeInput=(op:AiOperation,input:any)=>{if(!input||typeof input!=="object")return {};const allowed:Record<AiOperation,string[]>={INTERVIEW:["answer","difficulty","role"],LEARNING:["goal","timePerWeek"],APPLICATION_COPILOT:["jobTitle","jobDescription"],CAREER_COMPANION:["goal","blocker"]};return Object.fromEntries((allowed[op]||[]).filter(k=>typeof input[k]==="string"||typeof input[k]==="number").map(k=>[k,typeof input[k]==="string"?redactPII(String(input[k])):input[k]]));};
export async function runAiGateway(req:NextRequest, raw:string, input:any={}):Promise<GatewayResult>{const op=normalize(raw);if(!op)return {ok:false,status:400,message:"Opération IA inconnue."};const auth=await getAuthUser(req);if(!auth)return {ok:false,status:401,message:"Session requise."};const sb=adminClient();const user=await ensureUser(sb,auth);const {data:sub}=await sb.from("Subscription").select("planCode,status").eq("userId",user.id).in("status",["ACTIVE","TRIALING"]).order("createdAt",{ascending:false}).limit(1).maybeSingle();const plan=planOf(sub),cost=AI_OPERATION_COST[op],quota=AI_CREDITS_BY_PLAN[plan];

// Quota: la réservation est effectuée atomiquement par PostgreSQL après préparation du contexte.
// Ne pas refaire un SELECT + INSERT ici : deux requêtes concurrentes pourraient dépasser le quota.
const [pr,sr,er]=await Promise.all([sb.from("Profile").select("headline,summary,location,targetRoles,preferredSectors,targetCities").eq("userId",user.id).maybeSingle(),sb.from("Skill").select("name,level").eq("userId",user.id).limit(50),sb.from("Experience").select("title,company,description").eq("userId",user.id).limit(20)]);if(pr.error||sr.error||er.error)return {ok:false,status:500,message:(pr.error||sr.error||er.error)?.message||"Contexte indisponible."};const gaps:string[]=[];if(!pr.data?.targetRoles?.length)gaps.push("Définir un métier cible");if(!sr.data?.length)gaps.push("Ajouter des compétences");if(!er.data?.length)gaps.push("Ajouter une expérience");if(!pr.data?.summary)gaps.push("Compléter le résumé");
...
const context={profile:safeProfile,skills:sr.data||[],experiences:safeExperiences,gaps,readiness:Math.max(0,100-gaps.length*20),nextBestAction:gaps[0]||"Consulter les opportunités",input:safeInput};
const started=Date.now(),requestHash=sha({op,input:safeInput,context:{profile:context.profile,skills:context.skills,experiences:context.experiences}});
...
await sb.from("AuditLog").insert({userId:user.id,action:"AI_OPERATION",entityType:"AiUsage",metadata:{operation:op,credits:cost,provider:orchestration.provider,model:orchestration.model,attempts:orchestration.attempts,fallbackUsed:orchestration.fallbackUsed,requestHash}});
