...
export async function runAiGateway(req:NextRequest, raw:string, input:any={}):Promise<GatewayResult>{
  ...
  const gaps:string[]=[];if(!pr.data?.targetRoles?.length)gaps.push("Définir un métier cible");if(!sr.data?.length)gaps.push("Ajouter des compétences");if(!er.data?.length)gaps.push("Ajouter une expérience");if(!pr.data?.summary)gaps.push("Compléter le résumé");
  ...
  const context={profile:safeProfile,skills:sr.data||[],experiences:safeExperiences,memory,gaps,readiness:Math.max(0,100-gaps.length*20),nextBestAction:gaps[0]||"Consulter les opportunités",input:safeInput};
