...
function companyLogo(company: Job["company"]) {
  if (company?.logoUrl) return company.logoUrl;
  if (company?.website) {
    try {
      const host = new URL(company.website.startsWith("http") ? company.website : `https://${company.website}`).hostname.replace(/^www\\./, "");
      if (host && !["emploi.cm", "cameroondesks.com"].includes(host)) {
        return `https://www.google.com/s2/favicons?domain=${encodeURIComponent(host)}&sz=128`;
      }
    } catch {}
  }
  return companyAvatar(company?.name);
}
