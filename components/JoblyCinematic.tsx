...
function TalentPitchStudio({owner,pitchUrl,pitchDuration,busy,message,recording,setRecording,recordSeconds,setRecordSeconds,onUpload,onDelete,router}:{owner:any;pitchUrl:string|null;pitchDuration:number|null;busy:boolean;message:string|null;recording:boolean;setRecording:(v:boolean)=>void;recordSeconds:number;setRecordSeconds:(v:number)=>void;onUpload:(f:File)=>Promise<void>;onDelete:()=>Promise<void>;router:any}){
  ...
  const videoRef=React.useRef<HTMLVideoElement>(null); const timerRef=React.useRef<number|null>(null);
  ...
  async function start(){
    ...
    try{const s=await navigator.mediaDevices.getUserMedia({video:true,audio:true});setStream(s);if(videoRef.current){videoRef.current.srcObject=s;await videoRef.current.play();}
      ...
      setRec(mr);setRecording(true);setRecordSeconds(0);mr.start();const started=Date.now();timerRef.current=window.setInterval(()=>{const sec=Math.min(8,Math.floor((Date.now()-started)/1000));setRecordSeconds(sec);if(sec>=8)mr.stop()},200);
  ...
  return <main className="min-h-[100dvh] overflow-hidden bg-[#2E3F4F] pb-28 text-[#FFFEFB]"><BlobShape className="left-[-18%] top-[6%] h-[45vw] w-[45vw] bg-[#7A9BB5]"/><GiantOutline className="right-[-8%] top-24 text-[110px] font-black italic">PITCH</GiantOutline><div className="relative mx-auto max-w-3xl px-5 pt-7 sm:px-8"><button onClick={()=>router.back()} className="rounded-full border border-white/15 bg-white/10 px-4 py-2 text-xs font-bold">← Back</button><span className="mt-8 block text-[10px] font-black uppercase tracking-[2px] text-[#7A9BB5]">TALENT / VIDEO PITCH</span><h1 className="mt-2 text-[58px] font-black italic leading-[.8] tracking-[-.07em]">Show them<br/><span className="text-[#FFE135]">you.</span></h1><p className="mt-5 max-w-xl text-[13px] italic text-white/65">Crée un pitch de 5 à 8 secondes. Une seule prise. Ton visage, ton énergie, ton signal.</p><div className="mt-7 overflow-hidden rounded-[36px] border border-white/20 bg-white/[.08] p-3 backdrop-blur-[24px]">{recording?<video ref={videoRef} muted playsInline className="aspect-[9/16] max-h-[58vh] w-full rounded-[30px] bg-[#7A9BB5]/20 object-cover"/>:pitchUrl?<video src={pitchUrl} controls muted playsInline className="aspect-[9/16] max-h-[58vh] w-full rounded-[30px] bg-[#7A9BB5]/20 object-cover"/>:<div className="grid aspect-[9/16] max-h-[58vh] place-items-center rounded-[30px] bg-[#7A9BB5]/20 p-8 text-center"><div><div className="mx-auto grid h-20 w-20 place-items-center rounded-full bg-[#FFE135] text-[#2E3F4F]">●</div><p className="mt-5 text-sm font-black">Aucun pitch publié</p><p className="mt-1 text-xs text-white/55">Enregistre ou importe ta vidéo.</p></div></div>} {recording&&<div className="absolute ml-[-1px] mt-[-78px] rounded-full bg-[#2E3F4F]/85 px-4 py-2 text-[11px] font-black text-[#FFE135]">REC · {recordSeconds}s / 8s</div>}</div><div className="mt-5 grid gap-3 sm:grid-cols-2"><button disabled={busy||recording} onClick={start} className="rounded-full bg-[#FFE135] py-4 font-black text-[#2E3F4F]">{recording?"Recording…":"Record 5–8s pitch"}</button><label className={`cursor-pointer rounded-full border border-white/20 bg-white/10 py-4 text-center font-black ${busy||recording?"pointer-events-none opacity-50":""}`}>Upload video<input type="file" accept="video/mp4,video/webm,video/quicktime" className="hidden" disabled={busy||recording} onChange={e=>{const f=e.target.files?.[0];if(f)void onUpload(f);e.currentTarget.value=""}}/></label></div>{recording&&<button onClick={stop} className="mt-3 w-full rounded-full border border-white/20 bg-white/10 py-3 text-xs font-black">Stop recording</button>}{pitchUrl&&<div className="mt-5 flex items-center justify-between rounded-[26px] border border-white/15 bg-white/[.08] p-4"><div><span className="text-[9px] font-bold uppercase tracking-[1.5px] text-[#7A9BB5]">LIVE SIGNAL</span><p className="mt-1 text-sm font-black">Pitch publié · {pitchDuration?`${(pitchDuration/1000).toFixed(1)}s`:"durée validée"}</p></div><button disabled={busy} onClick={()=>void onDelete()} className="rounded-full border border-white/20 px-4 py-2 text-xs font-bold">Supprimer</button></div>}{(message||localError)&&<p className="mt-4 rounded-2xl border border-[#FFE135]/25 bg-[#FFE135]/10 p-3 text-xs font-bold text-[#FFE135]">{message||localError}</p>}<p className="mt-6 text-[10px] font-semibold uppercase tracking-[1.4px] text-white/45">MP4 · WEBM · MOV · 25MB MAX · 5–8S</p></div><Nav3D active="Profile" items={[{label:"Home",href:"/dashboard",icon:<Home size={16}/>},{label:"Matches",href:"/opportunities",icon:<Heart size={16}/>},{label:"Apply",href:"/jobs",icon:<Send size={16}/>},{label:"Messages",href:"/notifications",icon:<MessageCircle size={16}/>},{label:"Profile",href:"/talent/profile",icon:<UserRound size={16}/>}]} /></main>;
}

export function CinematicProfile() {
  const router=useRouter(); const params=useSearchParams();
  const [candidate,setCandidate]=useState<any|null>(null); const [owner,setOwner]=useState<any|null>(null); const [chat,setChat]=useState(false); const [menu,setMenu]=useState(false); const [playing,setPlaying]=useState(true); const [pitchUrl,setPitchUrl]=useState<string|null>(null); const [pitchDuration,setPitchDuration]=useState<number|null>(null); const [pitchBusy,setPitchBusy]=useState(false); const [pitchMessage,setPitchMessage]=useState<string|null>(null); const [recording,setRecording]=useState(false); const [recordSeconds,setRecordSeconds]=useState(0);
  const y=useMotionValue(0), sy=useSpring(y,{stiffness:200,damping:12}); const scale=useTransform(sy,[-300,0],[1.09,1]);
  useEffect(()=>{if(params.get("candidate"))return; getSupabaseClient().auth.getSession().then(async({data})=>{if(!data.session){router.replace("/");return;} const r=await fetch("/api/profile",{headers:{Authorization:`Bearer ${data.session.access_token}`}}); if(r.ok){const b=await r.json(); setOwner(b); setPitchUrl(b.user?.pitchVideoUrl||null); setPitchDuration(b.user?.pitchVideoDurationMs||null);}})},[params,router]);
  ...
  async function uploadPitch(file:File){
    ...
    if(durationMs<5000||durationMs>8000){setPitchMessage("Ton pitch doit durer entre 5 et 8 secondes.");return;}
  ...
  if(isOwner){return <TalentPitchStudio owner={owner} pitchUrl={pitchUrl} pitchDuration={pitchDuration} busy={pitchBusy} message={pitchMessage} recording={recording} setRecording={setRecording} recordSeconds={recordSeconds} setRecordSeconds={setRecordSeconds} onUpload={uploadPitch} onDelete={removePitch} router={router}/>;}
