...
export default function JiaPresence() {
  ...
  return <div className="pointer-events-none relative h-full w-full overflow-visible" aria-label="J’IA — présence intelligente de Jobly">
    <motion.div className="pointer-events-auto absolute bottom-3 right-3 h-[165px] w-[105px] max-w-[calc(100vw-24px)] cursor-grab touch-none sm:bottom-4 sm:right-4 sm:h-[210px] sm:w-[140px]" drag dragMomentum={false} dragElastic={0.06} dragConstraints={{ left: -Math.max(0, viewport.w - 300), right: 0, top: -Math.max(0, viewport.h - 430), bottom: 0 }} whileTap={{ cursor: "grabbing", scale: 0.99 }}>
