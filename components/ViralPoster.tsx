...
export default function ViralPoster({ job }: Props) {
  ...
  async function generate(){
    ...
    try {
      ...
      ctx.fillStyle="#FFC72C"; ctx.roundRect(70,135,940,14,7); ctx.fill();
      ...
      ctx.fillStyle="#2E5C9E"; ctx.font="700 34px Arial"; ctx.fillText(job.companyName||"Entreprise",70,y+80);
      ...
      ctx.fillStyle="#2E5C9E"; ctx.font="700 25px Arial"; ctx.fillText(url,70,800);
  ...
  return <button type="button" onClick={generate} disabled={busy} className="w-full rounded-full bg-[#FFC72C] px-5 py-3 text-sm font-black text-navy disabled:opacity-50">{busy?"Génération…":"Générer affiche virale"}{message&&<span className="ml-2 text-[10px]">{message}</span>}</button>;
}
