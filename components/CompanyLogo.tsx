...
export default function CompanyLogo({
  ...
  const candidates = useMemo(() => {
    ...
    if (resolvedDomain && logoDevToken) {
      urls.push(`https://img.logo.dev/${encodeURIComponent(resolvedDomain)}?token=${encodeURIComponent(logoDevToken)}&size=128`);
    }
    if (resolvedDomain) {
      urls.push(`https://www.google.com/s2/favicons?domain=${encodeURIComponent(resolvedDomain)}&sz=128`);
    }
    return urls;
