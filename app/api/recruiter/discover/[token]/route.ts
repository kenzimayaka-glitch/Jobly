...
function match(profile:any, expYears:number, job:any){
 const roles=(profile?.targetRoles||[]).map(norm), cities=(profile?.targetCities||[]).map(norm), sectors=(profile?.preferredSectors||[]).map(norm);
 const title=norm(job.title), loc=norm(job.location), sector=norm(job.sector), min=job.minExperienceYears||0;
 const role=roles.length&&roles.some((r:string)=>title.includes(r))?1:0;
 const city=cities.length&&cities.some((c:string)=>loc.includes(c))?1:0;
 const sec=sectors.length&&sector&&sectors.some((s:string)=>sector.includes(s))?1:0;
 const experience=expYears>=min?1:0;
 return Math.round(((role+city+sec+experience)/4)*100);
}
...
export async function GET(_request:NextRequest,{params}:{params:Promise<{token:string}>}) {
 try{
  ...
  const {data:profiles}=await db.from("Profile").select("userId,firstName,lastName,headline,summary,location,targetRoles,preferredSectors,publicDiscoverable").eq("publicDiscoverable",true).neq("userId",share.userId).limit(250);
  ...
  const talents=(profiles||[]).map((p:any)=>{const u=userBy.get(p.userId)||{};const e=expBy.get(p.userId)||[];const score=match(p,years(e),job);return{id:p.userId,name:[p.firstName,p.lastName].filter(Boolean).join(" ")||u.displayName||"Talent Jobly",headline:p.headline||"Profil professionnel",location:p.location||"",profilePhotoUrl:u.profilePhotoUrl||null,skills:(skillBy.get(p.userId)||[]).slice(0,6),experienceYears:years(e),matchPercent:score};}).sort((a,b)=>b.matchPercent-a.matchPercent).slice(0,12);
  return NextResponse.json({share:{jobTitle:share.jobTitleSnapshot,companyName:share.companyNameSnapshot},job,talents});
