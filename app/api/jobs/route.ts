...
export async function GET(request:NextRequest){
 try{
  ...
  return NextResponse.json({totalAvailable,matchingCount,count:results.length,page,limit,hasMore:start+limit<totalAvailable,yearsExperience,jobs:results,freshnessPolicy:{dailyTarget:200,matchingThreshold:50,expirationField:"expirationAt",ordering:"match_then_publication",externalRequiresApplicationReady:true}});
