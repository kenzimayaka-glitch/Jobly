...
const translations = {
  fr: {
    ...
    signupSubtitle: "Ces informations seront déjà enregistrées dans ton profil JOBLY.",
    lastName: "Nom",
    firstName: "Prénom",
  ...
  en: {
    ...
    signupSubtitle: "This information will already be saved in your JOBLY profile.",
    lastName: "Last name",
...
export default function Home() {
  ...
  return <main className={`relative h-[100dvh] w-full bg-off-white dark:bg-[#0A1931] dark:text-white ${(screen === "welcome" || screen === "signup") ? "overflow-hidden" : "overflow-y-auto"}`}>
    ...
    <div className="relative flex min-h-[100dvh] w-full flex-col gap-0">
        ...
        <AnimatePresence mode="wait" initial={false}>
          ...
          >
            <div className="relative min-h-0 flex-1 overflow-hidden px-4 pt-0">
              ...
              <div className="absolute bottom-0 right-[-1%] z-[5] h-[78%] w-[63%] sm:h-[76%] sm:w-[60%] lg:h-[70%] lg:w-[50%]">
                <Image
                  fill
                  priority
      ...
      {screen === "signup" && <section className="container mx-auto flex w-full max-w-[480px] flex-1 flex-col items-center justify-start overflow-y-auto px-4 pt-5 pb-4 text-center"><h1 className="w-full text-center text-[32px] font-extrabold text-navy">{translations[lang].signupTitle}</h1><p className="mb-3 w-full text-sm text-jobly-gray">{translations[lang].signupSubtitle}</p><div className="grid w-full grid-cols-2 gap-3"><label><span className="mb-1 block text-xs font-bold text-navy">{translations[lang].lastName}</span><input value={lastName} onChange={e=>setLastName(e.target.value)} autoComplete="family-name" className="h-12 w-full rounded-2xl border px-3" placeholder="BITSEKI"/></label><label><span className="mb-1 block text-xs font-bold text-navy">{translations[lang].firstName}</span><input value={firstName} onChange={e=>setFirstName(e.target.value)} autoComplete="given-name" className="h-12 w-full rounded-2xl border px-3" placeholder="MAYAKA"/></label></div><label className="mt-3 block w-full"><span className="mb-1 block text-xs font-bold text-navy">{translations[lang].phone}</span><div className="flex h-12 items-center rounded-2xl border px-2"><CountryPicker country={country} placeholder={translations[lang].countrySearch} onChange={(c)=>{setCountry(c);setPhone("")}}/><span className="mx-1 h-6 w-px bg-slate-200"/><input value={formatPhoneDisplay(phone,country.code)} onChange={e=>setPhone(e.target.value.replace(/\D/g,"").slice(0,15))} inputMode="tel" autoComplete="tel" placeholder={country.code==="CM"?"6 12 34 56 78":"Numéro"} className="w-full outline-none"/></div></label><label className="mt-3 block w-full"><span className="mb-1 block text-xs font-bold text-navy">{translations[lang].email}</span><input type="email" value={email} onChange={e=>setEmail(e.target.value)} inputMode="email" autoComplete="email" placeholder="vous@exemple.com" className="h-12 w-full rounded-2xl border px-3"/></label><button type="button" onClick={startSignup} disabled={busy} className="mt-5 h-[52px] w-full rounded-2xl bg-[#FFDE00] font-extrabold text-black">{busy?"Envoi du code…":translations[lang].continue}</button><p className="mt-4 w-full text-center text-xs text-slate-400">{translations[lang].phoneNote}</p></section>}
