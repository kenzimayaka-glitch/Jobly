...
import { connectGmail, disconnectGmail, getGmailConnection } from "../../../lib/gmailService";
...
export default function RecruiterSettingsPage() {
  ...
  useEffect(() => {
    ...
    if (gmailCallback === "1") {
      (async () => {
        const session = await (await import("../../../lib/supabase")).getSupabaseClient().auth.getSession();
        const email = session.data.session?.user.email || getGmailConnection().email;
        const connected = { connected: true, email: email || null, connectedAt: new Date().toISOString() };
        localStorage.setItem("jobly:gmail-connection", JSON.stringify(connected));
        setGmail(connected);
        setMessage("Google/Gmail connecté. Les scopes Gmail doivent être activés dans Google Cloud/Supabase pour lire/envoyer les emails.");
      })();
    }
    try {
      ...
      <div className="relative z-10">
        ...
        <div className="mx-auto max-w-3xl space-y-4 px-5 py-5">
          ...
          <section className="rounded-[24px] bg-white p-5 shadow-sm">
            ...
            <p className="mt-1 text-xs text-jobly-gray">Contrôle simulé des candidats convoqués depuis plus de 3 jours.</p>
          ...
          <section className="rounded-[24px] bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between gap-4">
              ...
              <span className={`rounded-full px-3 py-1 text-[10px] font-black ${gmail.connected ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-jobly-gray"}`}>
                {gmail.connected ? "CONNECTÉ" : "NON CONNECTÉ"}
              </span>
            </div>
            <button onClick={handleGmail} className="mt-4 w-full rounded-full bg-[#FFC72C] px-5 py-3 text-sm font-extrabold text-[#0A1931]">
              {gmail.connected ? "Déconnecter Gmail" : "Connecter mon Gmail"}
            </button>
          </section>
          ...
          <section className="rounded-[24px] bg-white p-5 shadow-sm">
            <div className="flex items-center justify-between gap-3">
              <div>
                <h2 className="font-heading text-base font-extrabold">Signature Jobly</h2>
                <p className="mt-1 text-xs text-jobly-gray">Ajoute automatiquement la signature à tes emails de recrutement.</p>
              </div>
              <button aria-label="Activer la signature" onClick={() => { const v=!signatureEnabled; setSignatureEnabled(v); persist({signatureEnabled:v}); }} className={`h-7 w-12 rounded-full p-1 ${signatureEnabled ? "bg-[#2E5C9E]" : "bg-slate-200"}`}>
                <span className={`block h-5 w-5 rounded-full bg-white transition-transform ${signatureEnabled ? "translate-x-5" : ""}`} />
              </button>
            </div>
